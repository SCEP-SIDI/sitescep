...
BUILDINGS = {
    "Maison principale": {
        ...
        "Bas": {
            "size": (800, 300),
            "rooms": [
                {"name": "Chambre Tom (bas)", "capacity": 2, "coords": (280, 60, 520, 240)},
            ],
        },
    },
    "Magnanerie": {
...
def generate_svg_plan(building, floor, occupation):
    ...
    parts = [f'<svg viewBox="0 0 {width} {height}" width="100%" xmlns="http://www.w3.org/2000/svg" '
              f'style="background:white;font-family:sans-serif;border:1px solid #ddd;">']
    ...
    for room in rooms:
        ...
        parts.append(f'<rect x="{x1}" y="{y1}" width="{w}" height="{h}" rx="{rx}" '
                      f'fill="{fill}" stroke="#333" stroke-width="2" />')

        cx = x1 + w / 2
        cy = y1 + 22
        parts.append(f'<text x="{cx}" y="{cy}" text-anchor="middle" font-size="13" '
                      f'font-weight="bold">{escape(room["name"])}</text>')

        if capacity is not None:
            parts.append(f'<text x="{cx}" y="{cy + 18}" text-anchor="middle" font-size="11" '
                          f'fill="#444">({len(occ)}/{capacity})</text>')
            for idx, nom in enumerate(occ):
                parts.append(f'<text x="{cx}" y="{cy + 38 + idx * 16}" text-anchor="middle" '
                              f'font-size="11">{escape(nom)}</text>')
...
def render_hebergement_tab():
    ...
    with sub_tab_plan:
        ...
        st.markdown(
            f"<span style='background:{COLOR_EMPTY};padding:2px 8px;border:1px solid #999;'>Vide</span>&nbsp;&nbsp;"
            f"<span style='background:{COLOR_PARTIAL};padding:2px 8px;border:1px solid #999;'>Partiel</span>&nbsp;&nbsp;"
            f"<span style='background:{COLOR_FULL};padding:2px 8px;border:1px solid #999;'>Complet</span>&nbsp;&nbsp;"
            f"<span style='background:{COLOR_NON_SLEEPING};padding:2px 8px;border:1px solid #999;'>Non habitable</span>",
            unsafe_allow_html=True,
        )
        svg, height = generate_svg_plan(building, floor, occupation)
        components.html(svg, height=height + 30, scrolling=True)
